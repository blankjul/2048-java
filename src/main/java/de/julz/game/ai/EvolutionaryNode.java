...
public class EvolutionaryNode extends GenericNode<Object> implements Comparable<EvolutionaryNode> {
	 ...
	 * @return
	 */
	public static EvolutionaryNode random(GameState stateObs, int pathLength, Evaluation eval) {
	...
	public EvolutionaryNode crossover(EvolutionaryNode p) {
		...
		for (int i = 0; i < path.size(); i++) {
			Action a = (new Random().nextDouble() < 0.5) ? path.get(i) : p.getPath().get(i);
			pathNew.add(a);
		}
		return new EvolutionaryNode(stateObs, pathNew, getEval());
	}
		...
		if (score == null) {
			...
			for (int j = 0; j < NUM_RUNS; j++) {
				...
				for (int i = 0; i < path.size(); i++) {
					...
					copy.next(a);
					weight *= weight;
					score += eval.getScore(copy) * weight;
				}
	...
	public String toString() {
		return String.format("Path: %s | score:%s ", Arrays.toString(path.toArray(new Action[path.size()])), score) + super.toString()
				+ stateObs.getBoard().toString();
	}
